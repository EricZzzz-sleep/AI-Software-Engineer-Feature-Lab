...
export function createApp(db: DatabaseSync, env: string | undefined) {
  ...
  function authenticate(req: IncomingMessage) {
    ...
    return { id: actor.id, name: actor.name, tokenHash: hash(token!) };
  }
      ...
      if (route) {
        ...
        if (method === 'PUT' && !action) {
          ...
          mutate(db, actor.id, id, 'save', await body(req), () => {
            if (fixturesEnabled && saveFailures.delete(failureKey)) throw new HttpError(500, 'Nothing was saved; your input has been kept. Injected failure before commit. Try Save again.');
          });
